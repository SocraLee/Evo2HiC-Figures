"""Supplementary Figure 9: Per-cell × per-track MSE for the Fig 4
epigenomic-prediction ablation (three-method comparison).

Bar layout mirrors [plot/plot_supp8.ipynb](plot_supp8.ipynb) — grouped
...
`Figures/statistics{,_table}.md` (Supp 9 section).
...
Outputs
-------
Figures/supplementary_9.pdf
Figures/supplementary_9_stats.tsv
"""
...
OUT_PDF = ensure_out_dir() / "supplementary_9.pdf"
OUT_TSV = ensure_out_dir() / "supplementary_9_stats.tsv"
OUT_WINCOUNT_TSV = ensure_out_dir() / "supplementary_9_mse_wincount.tsv"
...
def _report_mse_wincount(mse):
    """Count (cell x track) entries where Evo2HiC has strictly lower MSE than
    the second-best baseline (= per-entry min MSE over {Evo 2, HiC-only}).
    Prints an overall + per-cell summary and persists a per-entry TSV.

    `mse` : dict {(cell, track, method) -> scalar MSE}.
    """
    BASELINES = [m for m in METHODS if m != "Evo2HiC"]   # ['Evo 2', 'HiC-only']
    rows = []
    for cell in CELLS:
        for t in TRACKS:
            e = mse.get((cell, t, "Evo2HiC"), np.nan)
            cand = {m: mse[(cell, t, m)] for m in BASELINES
                    if np.isfinite(mse.get((cell, t, m), np.nan))}
            if not np.isfinite(e) or not cand:
                continue
            second_name = min(cand, key=cand.get)
            second_mse = cand[second_name]
            win = bool(e < second_mse)
            rows.append({"cell": cell, "track": t,
                         "mse_Evo2HiC": e,
                         "second_best_baseline": second_name,
                         "second_best_mse": second_mse,
                         "delta_mse": e - second_mse,
                         "evo2hic_wins": win})

    df = pd.DataFrame(rows)
    df.to_csv(OUT_WINCOUNT_TSV, sep="\t", index=False)
    print(f"[wrote] {OUT_WINCOUNT_TSV}")

    n_total = len(df)
    n_wins = int(df.evo2hic_wins.sum())
    indist = df[df.cell.isin(["GM12878", "H1ESC"])]
    print("\n=== MSE: Evo2HiC vs second-best baseline (min of Evo 2 / HiC-only) ===")
    print(f"  Overall:  {n_wins}/{n_total} entries where Evo2HiC has lower MSE")
    print(f"  In-distribution (GM12878+H1ESC): "
          f"{int(indist.evo2hic_wins.sum())}/{len(indist)}")
    for cell in CELLS:
        sub = df[df.cell == cell]
        if len(sub):
            print(f"  {cell:8s}: {int(sub.evo2hic_wins.sum())}/{len(sub)}")
    losses = df[~df.evo2hic_wins]
    if len(losses):
        print("  Losses:")
        for _, r in losses.iterrows():
            print(f"    {r.cell:8s} {r.track:10s}  "
                  f"(lost to {r.second_best_baseline}, "
                  f"ΔMSE={r.delta_mse:+.6f})")
...
def main():
    ...
    # ---- MSE win-count vs second-best baseline (paper reporting) ----
    # "Second-best" for each (cell, track) is the lower-MSE of the two
    # ablation baselines {Evo 2, HiC-only}. We report on how many of the 15
    # (cell x track) entries Evo2HiC achieves a strictly lower MSE than that
    # second-best comparator. This is the headline count cited in the paper;
    # per-entry relative MSE gaps are unstable (a single near-mean baseline
    # prediction on K562 H3K27me3 dominates any average), so we count wins
    # rather than average a ratio.
    _report_mse_wincount(mse)

    # ---- Plot — same layout as plot_supp8.ipynb cell 9 ----
    ...
    for ax, (cell, results) in zip(axes, all_results.items()):
        ...
        # x-tick labels per group (matches plot_supp8 last lines)
